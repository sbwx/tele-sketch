...
//----------------------------------------------------------------------------
  ...
  void pinMode(int_fast16_t pin, pin_mode_t mode) {
    ...
    gpio_set_direction((gpio_num_t)pin, (mode==pin_mode_t::output)?GPIO_MODE_OUTPUT:GPIO_MODE_INPUT);
    if(mode==pin_mode_t::input_pullup) gpio_set_pull_mode((gpio_num_t)pin, GPIO_PULLUP_ONLY);
    else if(mode==pin_mode_t::input_pulldown) gpio_set_pull_mode((gpio_num_t)pin, GPIO_PULLDOWN_ONLY);
}
